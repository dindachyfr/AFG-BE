const modelUsers = require("../model/user.js");
...
const registerUser = async (req, res, next) => {
    try {
        ...
        if (emailavailablitiy.length > 0) {
            ...
            commonHelper.response(res, [accountCreated], 201, "Account has been successfully created");
        }
    } catch (error) {
...
const loginUser = async (req, res, next) => {
    try {
        ...
        if (!passwordMatches) {
            ...
            commonHelper.response(res, userDisplay, 200, "Login Success");
        }
    } catch (error) {
        const errorRes = new Error("Internal Server Error");
        errorRes.status = 500;
